...
{
    public partial class frmbusqCompras : Form
    {
        public frmbusqCompras()
        {
            InitializeComponent();
        }

        
        private void frmbusqCompras_Load(object sender, EventArgs e)
        {
            this.Show();
        ...
        private void txtcodigo_KeyPress(object sender, KeyPressEventArgs e)
        {
            ...
            if (e.KeyChar == (char)Keys.Enter)
            {
                ...
                {
                    DataTable d = NCompra.LISTAR(int.Parse(txtcodigo.Text));

                    try
                    {
                        DataRow fila = d.Rows[0];
                        txtproveedor.Text = fila["Proveedor"].ToString().Trim();
                        datFecha.Value = DateTime.Parse(fila["Fecha"].ToString().Trim());
                        lbtotal.Text = string.Format("{0,-28:C2}",float.Parse(fila["Total"].ToString().Trim()));


                        gridview.AllowUserToAddRows = true;
                        gridview.DataSource = NDetalles_Compra.LISTAR(int.Parse(txtcodigo.Text));
